...
$(function() {
    ...
    $("#IranMap svg g path").hover(function() {
        ...
        var b = $(this).parent("a").parent("g").attr("class");
      console.log(c);
			console.log(b);
...
$(function() {
    ...
    function a() {
        var b = $("#IranMap2 .list").height();
        var c = $("#IranMap2 .list").width();
        if (b > c) {
            $("#IranMap2 svg").height(c).width(c)
        } else {
            $("#IranMap2 svg").height(b).width(b)
        }
    }
    a();
    $("#IranMap2 svg g path").hover(function() {
        var c = $(this).attr("class");
        var b = $(this).parent("a").parent("g").attr("class");
      console.log(c);
			console.log(b);
        var d = $("#IranMap2 .list ." + b + " ." + c + " a").html();
        if (d) {
            $("#IranMap2 .list ." + b + " ." + c + " a").addClass("hover");
            $("#IranMap2 .show-title").html(d).css({
                display: "block"
            })
        }
    }, function() {
        $("#IranMap2 .list a").removeClass("hover");
        $("#IranMap2 .show-title").html("").css({
            display: "none"
        })
    });
    $("#IranMap2 .list ul li ul li a").hover(function() {
        var e = $(this).parent("li").attr("class");
        console.log("hovered");
        var c = $(this).parent("li").parent("ul").parent("li").attr("class");
        var b = "#IranMap2 svg g." + c + " path." + e;
        var d = $(b).attr("class");
        $(b).attr("class", d + " hover")
    }, function() {
        var e = $(this).parent("li").attr("class");
    ...
    $("#IranMap2").mousemove(function(d) {
        ...
        if ($("#IranMap2 .show-title").html()) {
          console.log("show title")
            var f = $(this).offset();
            var b = (c - f.left + 25) + "px";
            var g = (h - f.top - 5) + "px";
            $("#IranMap2 .show-title").css({
                left: b,
                top: g
            })
        }
